...
import { FILM_PRICES } from './src/constants';

function createOptions(filmType?: any): ProcessingOptions {
    return {
...
const runTest = (width: number, height: number, thick: 5 | 8, filmType: string | undefined, expectedTotal: number, description: string) => {
    const dimensions: GlassDimensions = { width, height, thickness: thick as any };
    const options = createOptions(filmType);
...
// 926 -> 1.0m, 368 -> 0.4m. Area 0.4. Cost 0.4 * 4200 = 1680.
runTest(926, 368, 5, 'shatterproof', 1680, '926x368 Shatterproof');

// Case 2: 500x500, Glasstect (20000)
// 0.5 * 0.5 = 0.25. 0.25 * 20000 = 5000.
runTest(500, 500, 5, 'glasstect', 5000, '500x500 Glasstect');

// Case 3: 1000x1000, TN-200 (8500)
// 1.0 * 1.0 = 1.0. 8500.
runTest(1000, 1000, 5, 'tn200', 8500, '1000x1000 TN-200');

// Case 4: No film
runTest(500, 500, 5, undefined, 0, 'No film');

// Case 5: 8mm thickness (Multiplier should NOT apply to film)
// 926x368, Shatterproof. Expected 1680 (Same as 5mm).
runTest(926, 368, 8, 'shatterproof', 1680, '926x368 8mm (Should ignore multiplier)');
